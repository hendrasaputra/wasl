...
python3 build.py     # regenerate index.html (front door) and tree.html (whole tree)
...
`index.html` and `tree.html` are generated and committed so the repo is browsable without a
toolchain. Never hand-edit them — edit `build.py`, `template.html` (the front door: one person,
their thread, the books), `tools/tree_template.html` (the whole tree), or the JSONL.
